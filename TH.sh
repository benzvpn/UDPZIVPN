...
echo "[6/9] กำลังติดตั้งตัวจัดการผู้ใช้..."
cat > /usr/local/bin/zimenu <<'PYMANAGER'
#!/usr/bin/env python3
import json
import os
import re
...
def add_user():
    ...
    print(f"สถานะการเข้าใช้: {GREEN if ok else RED}{'เปิดใช้งาน' if ok else 'เซิร์ฟเวอร์ผิดพลาด'}{RESET}")
    rule(); pause()
...
def list_users():
    ...
    for i, (pw, meta) in enumerate(items, 1):
        ...
        print(f"    รหัสผ่าน  : {BOLD}{pw}{RESET}")
        print(f"    วันหมดอายุ: {fmt_ts(meta.get('expires_at'))} ({left(meta.get('expires_at'))})")
        print(f"    สถานะ    : {state_color(st)}{st}{RESET} | การเข้าใช้: {GREEN if allowed else RED}{'ได้' if allowed else 'ไม่ได้'}{RESET}")
        if i != len(items):
...
def online_clients():
    ...
    rule()
    print(f"{DIM}แสดงแต่ละ IP เพียงครั้งเดียว ZiVPN ไม่สามารถระบุรหัสผ่านที่ตรงกับ IP ได้แม่นยำ{RESET}")
    pause()
...
def menu_header():
    ...
    clear(); title("ตัวจัดการผู้ใช้ ZiVPN")
    up = service_active()
    print(f"เซิร์ฟเวอร์ {GREEN if up else RED}{'● ออนไลน์' if up else '● ออฟไลน์'}{RESET}   ผู้ใช้ {BOLD}{active_users}{RESET}   หมดอายุ {expired}")
    rule()
...
def menu():
    ...
    while True:
        ...
        print(f"{CYAN}[3]{RESET} รายการผู้ใช้")
        print(f"{CYAN}[4]{RESET} ต่ออายุ / เปลี่ยนวันหมดอายุ")
        print(f"{CYAN}[5]{RESET} ตรวจสอบสิทธิ์ผู้ใช้")
        print(f"{CYAN}[6]{RESET} ผู้ใช้ที่ออนไลน์")
        print(f"{CYAN}[7]{RESET} สถานะเซิร์ฟเวอร์")
        print(f"{CYAN}[8]{RESET} ทำความสะอาดผู้ใช้ที่หมดอายุ")
        print(f"{GRAY}[0] กลับ / ออก{RESET}")
        rule()
        ch = input(f"{BOLD}เลือก › {RESET}").strip()
        if ch=="1": add_user()
        ...
        elif ch=="7": server_status()
        elif ch=="8": cleanup()
        elif ch=="0": clear(); return
        else: time.sleep(.5)
...
if __name__ == "__main__":
    main()
PYMANAGER
chmod 700 /usr/local/bin/zimenu
...
[Service]
Type=oneshot
ExecStart=/usr/local/bin/zimenu --expire
EOF
...
echo "[7/9] กำลังติดตั้งเมนูต่างๆ..."
cat > /usr/local/bin/zivpnmenu <<'SERVERMENU'
#!/bin/bash
RESET="\033[0m"; BOLD="\033[1m"; GREEN="\033[92m"; RED="\033[91m"; CYAN="\033[96m"; YELLOW="\033[93m"; GRAY="\033[90m"
pause(){ echo; read -r -p "กด Enter เพื่อกลับ..." _; }
status(){
  if systemctl is-active --quiet zivpn; then echo -e "${GREEN}● ออนไลน์${RESET}"; else echo -e "${RED}● ออฟไลน์${RESET}"; fi
}
while true; do
  clear
  echo -e "${CYAN}${BOLD}==========================================${RESET}"
  echo -e "${CYAN}${BOLD}          เมนูเซิร์ฟเวอร์ ZIVPN           ${RESET}"
  echo -e "${CYAN}${BOLD}==========================================${RESET}"
  echo -n "สถานะเซิร์ฟเวอร์ : "; status
  echo "พอร์ต UDP       : 5667"
  echo "ช่วงพอร์ต       : 6000-19999/UDP"
  echo
  echo -e "${CYAN}[1]${RESET} ดูสถานะ"
  echo -e "${CYAN}[2]${RESET} เริ่ม ZiVPN"
  echo -e "${CYAN}[3]${RESET} หยุด ZiVPN"
  echo -e "${CYAN}[4]${RESET} รีสตาร์ท ZiVPN"
  echo -e "${CYAN}[5]${RESET} บันทึกล่าสุด"
  echo -e "${CYAN}[6]${RESET} แสดงไฟล์กำหนดค่า"
  echo -e "${GRAY}[0] กลับ / ออก${RESET}"
  echo
  read -r -p "เลือก > " c
  case "$c" in
    1)
      clear
      systemctl status zivpn --no-pager || true
      echo
      ss -ulnp | grep ':5667' || true
      echo
      iptables -t nat -L PREROUTING -n -v | grep '5667' || true
      pause;;
    2) systemctl start zivpn; sleep 1;;
    3) systemctl stop zivpn; sleep 1;;
    4) systemctl restart zivpn; sleep 1;;
    5) clear; journalctl -u zivpn -n 40 --no-pager; pause;;
    6) clear; cat /etc/zivpn/config.json; pause;;
    0) clear; exit 0;;
  esac
done
SERVERMENU
chmod 755 /usr/local/bin/zivpnmenu

cat > /usr/local/bin/menu <<'MAINMENU'
#!/bin/bash
RESET="\033[0m"; BOLD="\033[1m"; GREEN="\033[92m"; RED="\033[91m"; CYAN="\033[96m"; GRAY="\033[90m"
while true; do
  clear
  if systemctl is-active --quiet zivpn; then ST="${GREEN}● ออนไลน์${RESET}"; else ST="${RED}● ออฟไลน์${RESET}"; fi
  echo -e "${CYAN}${BOLD}==========================================${RESET}"
  echo -e "${CYAN}${BOLD}           เมนูหลัก ZIVPN                ${RESET}"
  echo -e "${CYAN}${BOLD}==========================================${RESET}"
  echo -e "สถานะเซิร์ฟเวอร์ : $ST"
  echo
  echo -e "${CYAN}[1]${RESET} ตัวจัดการผู้ใช้"
  echo -e "${CYAN}[2]${RESET} ตัวจัดการเซิร์ฟเวอร์"
  echo -e "${CYAN}[3]${RESET} รีสตาร์ท ZiVPN"
  echo -e "${GRAY}[0] ออก${RESET}"
  echo
  read -r -p "เลือก > " c
  case "$c" in
    1) zimenu;;
    2) zivpnmenu;;
    3) systemctl restart zivpn; sleep 1;;
    0) clear; exit 0;;
  esac
done
MAINMENU
...
echo "  menu       เมนูหลัก"
echo "  zimenu     ตัวจัดการผู้ใช้"
echo "  zivpnmenu  ตัวจัดการเซิร์ฟเวอร์"
echo
